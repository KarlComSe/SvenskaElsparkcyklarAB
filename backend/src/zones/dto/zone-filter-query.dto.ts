...
export class ZoneFilterQueryDto {
  ...
  @IsOptional()
  @IsEnum(['parking', 'charging', 'speed'], {
    message: 'type must be one of parking, charging, speed',
  })
  type?: 'parking' | 'charging' | 'speed';

  @ApiPropertyOptional({
    enum: ['bikes'],
    description: 'Includes filter for additional options',
  })
  @IsOptional()
  ...
  @IsEnum(['Göteborg', 'Jönköping', 'Karlshamn'], {
    message: 'city must be one of Göteborg, Jönköping, Karlshamn',
  })
